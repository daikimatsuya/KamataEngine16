...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	Plane plane{ {0.0f,1.1f,0.1f},0.4f };
	...
	while (Novice::ProcessMessage() == 0) {
		...
		/// ↓更新処理ここから
		///
		if (pc_.IsCollision(Sphere1, plane)) {
			color = 0xFF0000FF;
		}
		else {
			color = 0xFFFFFFFF;
		...
		ImGui::Begin("Window");
		ImGui::DragFloat3("Sphere.Translate", &Sphere1.center.x, 0.01f);
		...
		dsag_.DrwaSphere(Sphere1, worldViewProjectionMatrix, viewPortMatrix, color);
